  ...
  return (
    <div className={`text-center mt-t-sm ${className}`}>
      <button
        ...
        className="font-inter text-[13px] t-text-secondary underline underline-offset-[3px] bg-transparent border-none cursor-pointer transition-opacity duration-200 hover:text-t-creme/70"
      >
        {trigger}
      </button>
      {open && (
        <p className="font-inter text-sm t-text-secondary italic mt-2 leading-relaxed">
          {children}
        </p>
      )}
    </div>
